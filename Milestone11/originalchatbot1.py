...
def genrandom_answer(random_antworten_lst):
    xindex = -1
    antwort = random.choice(random_antworten_lst)
    loop = 0
    for element in random_antworten_lst:
        if antwort == element:
            xindex = loop
        loop += 1
    return xindex
...
def get_random_answer(xindex,random_antworten_lst):
    out = ""
    #out = random_antworten_lst[xindex]
    out = get_answer(xindex,random_antworten_lst)
    return out
...
def get_answer(xindex,antworten_lst):
    out = " "
    out = antworten_lst[xindex]
    return out
